...
from cnc import create_daily_distribution
from cnc.read_raw_data import read_raw_data


def save_to_client(name: str, data: Sequence[BaseModel]) -> None:
    with open(f"../src/cache/{name}.json", "w") as data_file:
        json.dump(
            [item.model_dump() for item in data], data_file, indent=4, default=str
        )
